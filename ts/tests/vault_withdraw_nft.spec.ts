...
describe("vault_withdraw_nft.ts", () => {
  it("success path builds Withdraw NFT instruction (discriminant=6) and sends tx", async () => {
    jest.resetModules();
    ...
    jest.isolateModules(() => {
      require("../cluster1/vault_withdraw_nft");
    });
    ...
    expect(ix.data[0]).to.eq(6);

    logSpy.mockRestore();
    errSpy.mockRestore();
  });

  it("error path logs error when ATA fetch fails", async () => {
    jest.resetModules();
