...
describe("vault_deposit.ts", () => {
  it("builds Deposit SOL instruction (discriminant=1) and sends tx", async () => {
    jest.resetModules();
    ...
    jest.isolateModules(() => {
      require("../cluster1/vault_deposit");
    });
    ...
    expect(ix.data.length).to.eq(9);
    expect(ix.data[0]).to.eq(1);
